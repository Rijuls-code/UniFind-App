...
const NeedBoardPage = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      // Mock AI processing - in production, this would call Gemini AI API
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // Fetch all products and do basic filtering
      const response = await productsAPI.getAll({ limit: 50 });
      const allProducts = response.data;
      
      // Simple keyword matching (in production, use Gemini AI for better matching)
      ...
      setResults({
        extracted: {
          ...
          budget: 'Flexible',
          condition: 'Any',
          features: ['Quality', 'Good condition']
        },
        matches: matches
      });
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 mb-8 shadow-sm">
            ...
            <Button
              ...
              className="w-full bg-gradient-to-r from-[#4f46e5] to-[#7c3aed] text-white font-semibold px-6 py-3 rounded-xl hover:opacity-90 transition-all duration-200 active:scale-95 disabled:opacity-50"
