...
export default function ListingDetailScreen({ route, navigation }) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      setProduct(data);
      if (data.seller_id) {
        const sellerData = await usersAPI.getById(data.seller_id);
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sellerCard}>
          <View style={styles.sellerHeader}>
            <View style={[styles.sellerAvatar, { alignItems: 'center', justifyContent: 'center', backgroundColor: '#E2E8F0' }]}>
              <Ionicons name="person" size={24} color="#94A3B8" />
            </View>
            <View style={styles.sellerInfo}>
              <Text style={styles.sellerName}>{seller?.name || product.seller_name}</Text>
              <Text style={styles.sellerCollege}>{seller?.college || product.seller_college}</Text>
            </View>
          ...
          <View style={styles.trustScoreContainer}>
            ...
            <Text style={styles.trustScore}>{seller?.trust_score || 0}%</Text>
          </View>
          ...
          <View style={styles.sellerStats}>
            ...
            <View style={styles.sellerStat}>
              <Text style={styles.sellerStatText}>{seller?.review_count || 0} reviews</Text>
            </View>
            <View style={styles.sellerStat}>
              <Text style={styles.sellerStatText}>
                Since {seller?.member_since ? new Date(seller.member_since).getFullYear() : 'N/A'}
              </Text>
            </View>
          </View>
        </View>
